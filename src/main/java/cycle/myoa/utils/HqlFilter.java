 ...
 * t#id 就是t.id
 * 
 * S:String L:Long I:Integer D:Date ST:Short BD:BigDecimal FT:Float B:Boolean
 * 
 * EQ 是操作符
 * 
		...
		if (name != null && value != null) {
			if (name.startsWith("QUERY_")) {// 如果有需要过滤的字段
				...
				if (filterParams.length == 4) {
					...
					if("t.cyOrganizationID".equals(columnName)){
						columnName="t.cyOrganization.id";
					}else if("t.cyUnitID".equals(columnName)){
						columnName="t.cyUnit.id";
					}
					
					
					if(StringUtils.equalsIgnoreCase(columnType, "B")){//布尔类型的情况
						hql.append(" and " + columnName+" ="+value+" ");
					}else{
					...
					System.out.println("value="+getObjValue(columnType, operator, value));
