...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	@Override
	public List<T> getByIds(Class<T> c, Serializable[] ids){
		if(ids==null||ids.length==0){
			return Collections.EMPTY_LIST;
		}else{
			return getCurrentSession().createQuery("from "+c.getSimpleName()+" r where r.id in (:ids)")
